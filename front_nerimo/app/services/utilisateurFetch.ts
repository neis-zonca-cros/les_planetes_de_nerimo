...
export async function updateUtilisateur(
  ...
  try {
    ...
    if (error.message === "Les mots de passe ne correspondent pas" || error.statusCode === 400) {
      Alert.alert("Erreur de mise à jour", "Les mots de passe ne correspondent pas ou d'autres erreurs se sont produites.");
    } else {
      Alert.alert("Erreur", "Une erreur s'est produite lors de la mise à jour des informations. Veuillez réessayer.");
    }
